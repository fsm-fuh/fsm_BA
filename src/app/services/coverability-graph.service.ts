...
*/
compareUserInputWithTargetState(userInputMarking: Record<string, number>, nextStateNode: CoverabilityStateNode): boolean {
let comparison = true;
...
//TODO anpassen für Omega-Erkennung
getCorrectUserMarking(node: CoverabilityStateNode, onCorrect: () => void, startMarking?: Record<string, number>): void {
...
const { graph, Q, processedNodeIds, nodeByLabel, counters } = this.initializeCoverabilityGraphCalculation(diagram);
...
if (!edgeExists) {
counters.edgeId++;
const edge = new CoverabilityFiringEdge(`Edge${counters.edgeId}`, source.id, target.id, transition.label, '');
...
const missingReachableEdges: { source: CoverabilityStateNode; target: CoverabilityStateNode; edge: CoverabilityFiringEdge }[] = [];
